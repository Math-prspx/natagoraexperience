window.addEventListener('DOMContentLoaded', async () => {
  ...
  const newReserveBtn = document.getElementById('newReserveBtn');
  ...
  function showForm() {
    reserveFormPanel.hidden = false;
    reserveFormPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  function hideForm() {
    reserveFormPanel.hidden = true;
  ...
  function uploadImageFile(file) {
    ...
    const token = window.AdminV2.getAdminToken ? window.AdminV2.getAdminToken() : (localStorage.getItem('admin_token') || '');
  ...
  async function loadMeta() {
              ...
              <tr>
                ...
                <td>
                  <div class="table-actions">
                    ...
                    <button type="button" class="btn btn-danger js-delete-place" data-place-id="${place.id}" data-place-name="${escapeHtml(place.name_fr || '')}">Supprimer</button>
                  </div>
                </td>
              </tr>
            `;
    ...
    document.querySelectorAll('.js-delete-place').forEach((button) => {
      button.addEventListener('click', async () => {
        ...
        if (!confirm(`Supprimer la reserve « ${placeName} » ? Cette action est irreversible.`)) return;

        try {
          await apiFetch('/admin/places/' + placeId, { method: 'DELETE' });
          await loadMeta();
          setStatus(statusEl, `Reserve « ${placeName} » supprimee.`);
        } catch (error) {
          setStatus(statusEl, error.message, true);
        }
      });
